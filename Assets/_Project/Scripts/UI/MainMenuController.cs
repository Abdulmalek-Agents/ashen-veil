...
namespace InventixGames.UI
{
    public class MainMenuController : MonoBehaviour
    {
        ...
        {
            var save = ServiceLocator.Get<ISaveService>();
            foreach (var m in database.missions) if (!save.IsMissionComplete(m.missionId)) { ServiceLocator.Get<IMissionService>().StartMission(m.missionId); return; }
        ...
        private void OnNewGame() { var save = ServiceLocator.Get<ISaveService>(); save.Data.completedMissionIds.Clear(); save.Save(); if (database.missions.Count > 0) ServiceLocator.Get<IMissionService>().StartMission(database.missions[0].missionId); }
        private void OnQuit() {
#if UNITY_EDITOR
